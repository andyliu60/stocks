...
for stock in stocks:
    ...
    if stock_trade_day != today:
        Notification(value1 = stock['name'], value2 = '数据没有更新', event = ifttt_event).sent()
    else:
        stock_price = stock_df.close[0]
        cost_pchange = round((stock_price - stock['cost']) / stock['cost'] * 100, 2)
        stock_pchange = '%+.2f%%' % round(stock_df.pct_chg[0],4)
        Notification(value1 = stock['name'], value2 = '收盘价：%.2f' % stock_price + ' | ' + '涨跌幅：' + stock_pchange + ' |  ' + '盈亏：%+.2f%%' % cost_pchange, event = ifttt_event).sent()
