...
stocks = ({'name':'永辉超市', 'symbol':'601933.SH', 'event':'yhcs_close'},{'name':'大华股份', 'symbol':'002236.SZ', 'event':'dhgf_close'})
...
for stock in stocks:
    ...
    if stock_trade_day != today:
        Notification(value1 = '数据没有更新', event = stock['event']).sent()
    else:
        stock_price = stock_df.close[0]
        stock_pchange = '%+.2f%%' % round(stock_df.pct_chg[0],4)
        Notification(value1 = '收盘价：%.2f' % stock_price, value2 = '涨跌幅：' + stock_pchange, event = stock['event']).sent()
